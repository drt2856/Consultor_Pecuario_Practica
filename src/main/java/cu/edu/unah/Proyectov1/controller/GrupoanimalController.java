...
import cu.edu.unah.Proyectov1.service.GrupoanimalService;
...
public class GrupoanimalController {
	...
	@Autowired
	private GrupoanimalService GrupoAnimal;
	
	@GetMapping(path= {"/findAll"},produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<List<Grupoanimal>> findAll() {
		try {
			return new ResponseEntity<List<Grupoanimal>>(GrupoAnimal.findAll(), HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<Grupoanimal> findById(
						...
						@PathVariable int id) {
		try {
			return new ResponseEntity<Grupoanimal>(GrupoAnimal.findgrupoanimal(id), HttpStatus.OK);
	...
	public ResponseEntity<Grupoanimal> create(
		...
		Grupoanimal result = GrupoAnimal.save(univ);
	...
	public ResponseEntity<Grupoanimal> updateGrupoanimal(
		...
		try {
			Grupoanimal result = GrupoAnimal.update(univ);
